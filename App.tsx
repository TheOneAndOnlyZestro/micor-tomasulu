...
import { DEFAULT_CONFIG, INITIAL_REGISTERS, SAMPLE_CODE_SEQUENTIAL, SAMPLE_CODE_LOOP } from './constants';
...
const App: React.FC = () => {
  ...
  const startSimulation = () => {
    try {
      const { instructions, labels: parsedLabels } = parseAssembly(code);
      // Pre-load some memory for demo (e.g., array values)
      // For this simulator, we just assume memory locations are usable.
      // R1, R2 usually hold addresses.
      // Let's seed initial registers with some reasonable defaults for the samples
      const initialRegs = INITIAL_REGISTERS.reduce((acc, name) => {
          acc[name] = 0;
          if (name === 'R1') acc[name] = 0;
          if (name === 'R2') acc[name] = 0;
          return acc;
      }, {} as Record<string, number>);

      // Set F2 to 1.33 for Sample Loop
      initialRegs['F2'] = 1.33; 
      ...
      setError("Failed to parse assembly code. Check syntax.");
      console.error(e);
    }
  };
    ...
    <div className="flex flex-col h-screen bg-gray-900 text-gray-100 font-sans">
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-gray-800 border-b border-gray-700 shadow-md z-10">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-xl font-bold tracking-tight">Tomasulo Simulator</h1>
        </div>
        ...
        <div className="flex items-center space-x-4">
           ...
           <button 
             ...
             className={`p-2 rounded hover:bg-gray-700 transition ${showConfig ? 'bg-gray-700 text-blue-400' : 'text-gray-400'}`}
             title="Configuration"
           >
        ...
        <div className="w-96 flex flex-col border-r border-gray-700 bg-gray-900 z-10 shadow-xl">
            {showConfig ? (
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    <h2 className="text-lg font-bold text-gray-300 border-b border-gray-700 pb-2">Configuration</h2>
                    
                    {/* Latencies */}
                    <div className="space-y-3">
                        <h3 className="text-sm uppercase text-gray-500 font-bold">Latencies (Cycles)</h3>
                        {Object.keys(config.latencies).map(op => (
                            <div key={op} className="flex justify-between items-center">
                                ...
                                <input 
                                    ...
                                    onChange={(e) => updateLatency(op, parseInt(e.target.value) || 1)}
                                />
                            </div>
                        ))}
                    </div>
                    ...
                    {/* Cache */}
                    <div className="space-y-3">
                        <h3 className="text-sm uppercase text-gray-500 font-bold">Cache</h3>
                        <div className="flex justify-between items-center">
                            ...
                            <input 
                                ...
                                onChange={(e) => updateCache('blockSize', parseInt(e.target.value) || 4)}
                            />
                        </div>
                        <div className="flex justify-between items-center">
                            ...
                            <input 
                                ...
                                onChange={(e) => updateCache('cacheSize', parseInt(e.target.value) || 16)}
                            />
                        </div>
                         <div className="flex justify-between items-center">
                            ...
                            <input 
                                ...
                                onChange={(e) => updateCache('missPenalty', parseInt(e.target.value) || 10)}
                            />
                        </div>
                    </div>
                ...
                <div className="flex-1 flex flex-col p-4 space-y-4">
                    <div className="flex justify-between items-center">
                        <h2 className="text-lg font-bold text-gray-300">Assembly Code</h2>
                        <div className="flex space-x-2">
                             <button onClick={() => { setCode(SAMPLE_CODE_SEQUENTIAL); handleReset(); }} className="text-xs bg-gray-800 hover:bg-gray-700 px-2 py-1 rounded text-gray-400">Seq</button>
                             <button onClick={() => { setCode(SAMPLE_CODE_LOOP); handleReset(); }} className="text-xs bg-gray-800 hover:bg-gray-700 px-2 py-1 rounded text-gray-400">Loop</button>
                        </div>
                    </div>
                    <textarea 
                ...
                <div className="p-4 border-t border-gray-700 bg-gray-800">
                    {!simState ? (
                        <button 
                            ...
                            className="w-full flex items-center justify-center space-x-2 bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded transition-all"
                        >
                        ...
                        <div className="flex space-x-2">
                            <button 
                                ...
                                className={`flex-1 flex items-center justify-center space-x-2 bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded transition-all ${simState.isFinished ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
        ...
        <div className="flex-1 bg-gray-900 p-6 overflow-hidden relative">
            {simState ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-600">
                    ...
                    <p className="text-lg">Load code and start simulation to see data.</p>
                </div>
            )}
        </div>
      </main>
